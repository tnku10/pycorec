...
#    本モードを使用する場合，ボタン横のテキストボックスにスキップ数(初期値2)を入力してからボタンを押してください。
#    例)読み込みたい画像が001,003,005の場合スキップ数＞2, 001,004,007の場合スキップ数＞3を指定
...
# ● スキップ数の仕様を変更 例)読み込みたい画像が001,003,005の場合スキップ数＞2
...
root = tk.Tk()
# ウインドウのタイトル
root.title('pycorec1.0.3')
# ウインドウサイズと位置指定 幅,高さ,x座標,y座標
root.geometry('300x300+0+0')
# フレームの作成
frame = tk.Frame(root, width=280, height=280, bg='#D9D9D9')
