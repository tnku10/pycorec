...
class PyCorec(customtkinter.CTk):
    def __init__(self):
        ...
        # フレームの作成
        self.frame = customtkinter.CTkFrame(self, width=280, height=780, bg='#D9D9D9')
        self.frame.place(x=10, y=10)
        self.frame_scale = customtkinter.CTkFrame(self.frame, relief=customtkinter.CTkFLAT, bg='#E6E6E6', bd=2)
        self.frame_scale.place(x=5, y=30, width=270, height=70)
        self.frame_display = customtkinter.CTkFrame(self.frame, relief=customtkinter.CTk.FLAT, bg='#E6E6E6', bd=2)
        self.frame_display.place(x=5, y=145, width=270, height=70)
        self.frame_menu = customtkinter.CTkFrame(self.frame, relief=customtkinter.CTk.FLAT, bg='#E6E6E6', bd=2)
        self.frame_menu.place(x=5, y=250, width=270, height=85)
        self.frame_save = customtkinter.CTkFrame(self.frame, relief=customtkinter.CTk.FLAT, bg='#E6E6E6', bd=2)
        self.frame_save.place(x=5, y=380, width=270, height=30)
        self.frame_tree = customtkinter.CTkFrame(self.frame, relief=customtkinter.CTk.FLAT, bg='#E6E6E6', bd=2)
        self.frame_tree.place(x=5, y=520, width=270, height=150)

        # 画像時空間パラメーターの入力
        self.tsparmlabel = customtkinter.CTkLabel(text='画像時空間パラメーターの入力(オプション項目)', bg='#D9D9D9')
        self.tsparmlabel.place(x=15, y=15)
        # fps指定用テキストボックスの作成
        self.fpstxtbox = customtkinter.CTkEntry(width=7)
        self.fpstxtbox.place(x=120, y=50)
        self.fpslabel = customtkinter.CTkLabel(text='フレームレート(fps)', bg='#E6E6E6')
        self.fpslabel.place(x=25, y=50)
        # 物理座標変換スケール(cm/px)指定用テキストボックスの作成
        self.scaletxtbox = customtkinter.CTkEntry(width=7)
        self.scaletxtbox.place(x=185, y=80)
        self.scalelabel = customtkinter.CTkLabel(text='物理座標変換スケール(cm/px)', bg='#E6E6E6')
        self.scalelabel.place(x=25, y=80)

        # 画像の読み込みに関するパラメーター入力とモード選択
        self.dpparmlabel = customtkinter.CTkLabel(text='画像読み込みパラメーターの指定', bg='#D9D9D9')
        self.dpparmlabel.place(x=15, y=130)
        # 表示画像サイズ縮小のための倍率指定用テキストボックスの作成
        self.pictxtbox = customtkinter.CTkEntry(width=7)
        self.pictxtbox.insert(0, '0.5')
        self.pictxtbox.place(x=130, y=165)
        self.piclabel = customtkinter.CTkLabel(text='画像の表示倍率', bg='#E6E6E6')
        self.piclabel.place(x=25, y=165)
        # フレームインターバル(画像読み込み間隔)の指定用テキストボックスの作成
        self.intervaltxtbox = customtkinter.CTkEntry(width=7)
        self.intervaltxtbox.insert(0, '1')
        self.intervaltxtbox.place(x=130, y=195)
        self.intervallabel = customtkinter.CTkLabel(text='フレームインターバル', bg='#E6E6E6')
        self.intervallabel.place(x=25, y=195)
        self.noticelabel = customtkinter.CTkLabel(text='(1=連番読込)', bg='#E6E6E6')
        self.noticelabel.place(x=185, y=195)
        # 画像の読み込み
        self.modelabel = customtkinter.CTkLabel(text='画像読み込み方法の選択', bg='#D9D9D9')
        self.modelabel.place(x=15, y=235)

        # データの出力
        self.outputlabel = customtkinter.CTkLabel(text='データの出力', bg='#D9D9D9')
        self.outputlabel.place(x=15, y=365)

        # 操作方法の記述
        self.guidelabel = customtkinter.CTkLabel(text='操作方法', bg='#D9D9D9')
        self.guidelabel.place(x=15, y=430)
        self.leftlabel = customtkinter.CTkLabel(text='左クリック: 座標記録(複数点記録可能)', bg='#D9D9D9')
        self.leftlabel.place(x=15, y=450)
        self.rightlabel = customtkinter.CTkLabel(text='右クリック: 記録済み座標を削除(同一画像中のみ)', bg='#D9D9D9')
        self.rightlabel.place(x=15, y=470)
        self.keylabel = customtkinter.CTkLabel(text='矢印右キー: 次の画像に移動', bg='#D9D9D9')
        self.keylabel.place(x=15, y=490)
        self.centerlabel = customtkinter.CTkLabel(text='マウスホイール押し込み: 強制終了', bg='#D9D9D9')
        self.centerlabel.place(x=15, y=510)

        # ボタン作成
        self.button = customtkinter.CTkButton(self.frame_menu, text='フォルダ選択(フォルダ内画像一括選択)', command=self.get_dir)
        self.button.grid(row=1, column=0, sticky=customtkinter.CTk.W)
        self.button = customtkinter.CTkButton(self.frame_menu, text='ファイル選択(連続範囲選択)', command=self.get_range)
        self.button.grid(row=14, column=0, sticky=customtkinter.CTk.W)
        self.button = customtkinter.CTkButton(self.frame_menu, text='ファイル選択(複数・任意選択可能)', command=self.get_files)
        self.button.grid(row=28, column=0, sticky=customtkinter.CTk.W)
        self.button_csv = customtkinter.CTkButton(self.frame_save, text='csvファイル保存', command=self.save_csv)
        self.button_csv.grid(row=1, column=0, sticky=customtkinter.CTk.W)
        # button_png = customtkinter.CTk.Button(frame_save, text='時系列座標グラフ保存', command=csv)
        # button_png.grid(row=1, column=1, sticky=customtkinter.CTk.W)

        # 表形式tree
        self.treeview = MyScrolledTreeview(self.frame)
        self.treeview.pack(padx=5, pady=530, anchor=customtkinter.CTk.W, fill=customtkinter.CTk.X)
